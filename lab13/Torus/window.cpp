...
void Window::Initialize(int major_gl_version, int minor_gl_version){
    ...
    view_matrix_.Translate(0, -3, -11);
    view_matrix_.RotateAboutX(10);
    SetViewMatrix();
...
void Window::InitModels(){
    ...
    plane_.SetTexture(cosmos_texture_);
    plane_.SetTextureUnit(GL_TEXTURE0);

    torus_.Initialize(kTorusM, kTorusN, kTorusR, kTorus_r);
    torus_.SetTexture(saturn_texture_);
    torus_.SetTexturePrim(saturn_texture_prim_);
